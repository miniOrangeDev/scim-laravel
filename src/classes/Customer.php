...
class CustomerSCIM
{
    ...
    function create_customer()
    {
        ...
        $url = DB::get_option('mo_scim_host_name') . '/moas/rest/customer/add';

        $ch = curl_init($url);
        $this->email = CD::get_option('mo_scim_admin_email');
        $password = CD::get_option('mo_scim_admin_password');
        // $password = $_POST['password'];

        $fields = array(
            'companyName' => $_SERVER ['SERVER_NAME'],
            'areaOfInterest' => 'Laravel SCIM Package',
            'email' => $this->email,
            'password' => $password
        );
    ...
    {
        $fromEmail = $email;
        $subject = "Laravel SCIM free Support Query - " . $email;
    ...
    {
        $fromEmail = $email;
        $subject = "Laravel SCIM Trial/Demo Request - " . $email;

        $content = '<div >Hello, <br><br>Company :<a href="' . $_SERVER['SERVER_NAME'] . '" target="_blank" >' . $_SERVER['SERVER_NAME'] . '</a><br><br>Email :<a href="mailto:' . $fromEmail . '" target="_blank">' . $fromEmail . '</a><br><br>Query: ' . $query . '</div>';
    ...
    {
        $fromEmail = $email;
        $subject = "Laravel SCIM free Customer Registration - " . $email;

        $content = '<div >Hello, <br><br>Company :<a href="' . $_SERVER['SERVER_NAME'] . '" target="_blank" >' . $_SERVER['SERVER_NAME'] . '</a><br><br>Email :<a href="mailto:' . $fromEmail . '" target="_blank">' . $fromEmail . '</a><br><br>Use Case: ' . $use_case . '</div>';
    ...
    {
        $url = DB::get_option('mo_scim_host_name') . "/moas/rest/customer/check-if-exists";
        $ch = curl_init($url);
        $email = CD::get_option("mo_scim_admin_email");
    ...
    {
        $url = DB::get_option('mo_scim_host_name') . "/moas/rest/customer/key";
        // dd($url);
        $ch = curl_init($url);
        // $email = CD::get_option("mo_scim_admin_email");
    ...
    function mo_scim_vl($code, $active)
    {
        $url = "";
        if ($active)
            $url = DB::get_option('mo_scim_host_name') . '/moas/api/backupcode/check';
        else
            $url = DB::get_option('mo_scim_host_name') . '/moas/api/backupcode/verify';

        $ch = curl_init($url);

        /* The customer Key provided to you */
        $customerKey = CD::get_option('mo_scim_admin_customer_key');

        /* The customer API Key provided to you */
        $apiKey = CD::get_option('mo_scim_admin_api_key');
    ...
    function check_customer_ln()
    {
        ...
        $url = DB::get_option('mo_scim_host_name') . '/moas/rest/customer/license';
        $ch = curl_init($url);
        $customerKey = CD::get_option('mo_scim_admin_customer_key');
    ...
    function remove_account() {
        ...
        if ( !CD::get_option('mo_scim_registration_status') || false === $code || empty( $code ) ) {
            return;
        }
        $host_name    = DB::get_option('mo_scim_host_name');
        $url          = $host_name . '/moas/api/backupcode/updatestatus';
        $customer_key = CD::get_option('mo_scim_admin_customer_key');
        $api_key      = CD::get_option('mo_scim_admin_api_key');
