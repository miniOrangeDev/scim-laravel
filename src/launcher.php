<?php
/*
 * Plugin Name: miniOrange Laravel SCIM
 * Version: 1.0.0
 * Author: miniOrange
 */
if (!isset($_SESSION)) {
...
if (!is_dir($_SERVER['DOCUMENT_ROOT'] . '/miniorange/sso')) {
    ...
    foreach ($file_paths_array as $path) {
        $src = __DIR__ . $path;
        $dst = public_path() . "/miniorange/mo_scim_free" . $path;
        mo_scim_recurse_copy($src, $dst);
    }
}
if (isset($_SESSION)) {
    if (mo_scim_is_user_registered() == NULL) {
        header("Location: mo_scim_register.php");
        exit();
    } else {
