...
if(isset($_REQUEST['option']) and $_REQUEST['option'] == 'testattrmappingconfig'){
    ...
    // exit;
    $mo_scim_app_name = sanitize_text_field($_REQUEST['app']);
    // header("Location: https://www.google.com/");
    // wp_redirect(CD::oauth_get_current_domain().'?option=oauthredirect&app_name='. urlencode($mo_scim_app_name)."&test=true");
 
}
if(isset($_POST['option']) and $_POST['option'] == 'test_config'){
    // exit;
    var_dump($_POST);exit;
    // $mo_scim_app_name = $_POST['displayappname'];
    // header("Location: https://www.google.com/");
    // wp_redirect(CD::oauth_get_current_domain().'?option=oauthredirect&app_name='. urlencode($mo_scim_app_name)."&test=true");
    // exit();
}
...
if(isset($_POST['option']) && $_POST['option'] == 'reset_config'){
    ...
    DB::update_option('mo_scim_message', "Settings reset successfully!");
}
...
if (isset($_POST['option']) && $_POST['option'] == 'save_connector_settings') {
        ...
        if(isset($_POST['discovery']) && !empty($_POST['discovery']) && !is_null($_POST['discovery'])){
            ...
            if((filter_var($discovery_endpoint, FILTER_VALIDATE_URL))){      
                ...
                if($content){
                    ...
                    $_POST['mo_scim_scopes_list'] = $scope_list;  
                    $scope = mo_scim_get_scopes($scope);
                    $_POST['mo_scim_scopes'] = $scope;
    ...
    DB::update_option('mo_scim_provider_config',json_encode($_POST));
    DB::update_option('mo_scim_message', 'Settings saved successfully!!');
    ...
    function mo_scim_get_scopes($scopes){
