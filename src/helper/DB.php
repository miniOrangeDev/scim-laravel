...
class DB extends Controller
{
    ...
    {
        try {
            $result = LaraDB::select('select * from mo_scim_config where mo_scim_domain_name = ?', [self::oauth_get_current_domain()]);
            if( count($result) == 0 ) {
                header('Location: mo_scim_create_tables');
                exit;
            } else {
                $result = $result[0];
            }
    ...
    {
        try {
            $result = LaraDB::table('mo_scim_config')->updateOrInsert([
                'mo_scim_domain_name' => self::oauth_get_current_domain()
            ], [
                $key => $value
            ]);
        } catch (\Exception $e) {
    ...
    {
        try {
            $result = LaraDB::select('select * from mo_scim_admin where mo_scim_domain_name = ?', [self::oauth_get_current_domain()]);
            if( count($result) == 0 ) {
                header('Location: mo_scim_create_tables');
                exit;
            } else {
    ...
    {
        try {
            LaraDB::table('mo_scim_admin')->updateOrInsert([
                'mo_scim_domain_name' => self::oauth_get_current_domain()
            ], [
                'email' => $email,
                'password' => $password
    ...
    public static function get_column($table, $key)
    {
        ...
        $option = ConDB::table($table)->where(['mo_scim_domain_name' => self::oauth_get_current_domain()])->first()->$key;
        return $option;
    }
    ...
    protected static function startConnection()
    {
        ...
        try {
            if (ConDB::table('mo_scim_config')->get() == NULL) {
                ConDB::table('mo_scim_config')->updateOrInsert([
                    'mo_scim_domain_name' => self::oauth_get_current_domain()
                ], [
                    'mo_scim_host_name' => 'https://login.xecurify.com'
                ]);
            }
        } catch (PDOException $e) {
